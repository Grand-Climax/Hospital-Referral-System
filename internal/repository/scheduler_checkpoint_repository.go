...
func (r *schedulerCheckpointRepository) GetNextEligibleDepartment(ctx context.Context, minAge time.Duration, leaseHolder string, leaseDuration time.Duration) (*entity.SchedulerCheckpoint, error) {
	...
	query := `
		...
		WHERE scheduler_checkpoints.hospital_id = next_dept.hospital_id 
		  AND scheduler_checkpoints.dept_id = next_dept.dept_id
...
func (r *schedulerCheckpointRepository) UpdateLastProcessed(ctx context.Context, hospitalID, deptID uuid.UUID, leaseHolder string) error {
	return r.db.WithContext(ctx).Model(&entity.SchedulerCheckpoint{}).
		Where("hospital_id = ? AND dept_id = ? AND lease_holder = ?", hospitalID, deptID, leaseHolder).
...
func (r *schedulerCheckpointRepository) ReleaseLease(ctx context.Context, hospitalID, deptID uuid.UUID, leaseHolder string) error {
	return r.db.WithContext(ctx).Model(&entity.SchedulerCheckpoint{}).
		Where("hospital_id = ? AND dept_id = ? AND lease_holder = ?", hospitalID, deptID, leaseHolder).
		Updates(map[string]interface{}{
			"lease_holder":     nil,
...
func (r *schedulerCheckpointRepository) AcquireLease(ctx context.Context, hospitalID, deptID uuid.UUID, leaseHolder string, leaseDuration time.Duration) (bool, error) {
	result := r.db.WithContext(ctx).Model(&entity.SchedulerCheckpoint{}).
		Where("hospital_id = ? AND dept_id = ? AND (lease_expires_at IS NULL OR lease_expires_at < NOW())", hospitalID, deptID).
		Updates(map[string]interface{}{
