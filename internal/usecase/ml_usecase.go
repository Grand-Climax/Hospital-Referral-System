...
func (u *mlUseCase) scoreReferral(ctx context.Context, referralID uuid.UUID, force bool) error {
	...
	for attempt := 0; attempt < u.maxRetries; attempt++ {
		scoreResp, lastErr = u.client.Score(ctx, scoreReq)
		if lastErr == nil {
			break
		}
	...
	txErr := u.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		pred := &entity.MLPrediction{
			...
			ExternalPredictionID: &extID,
			TriggerReason:        trigger,
			InputFeatures:        inputJSON,
			OutputScore:          scoreResp.SeverityScore,
			SeverityTier:         &tier,
		...
		return tx.Model(&entity.Referral{}).Where("id = ?", referralID).Updates(map[string]interface{}{
			...
			"ml_severity_score":       scoreResp.SeverityScore,
...
func (u *mlUseCase) shouldScoreReferral(ref *entity.Referral, force bool) bool {
	...
	// Recover referrals stuck PENDING when liaison/specialist moved status before async scoring ran.
	if ref.MLStatus == entity.MLStatusPending || ref.MLStatus == entity.MLStatusFailed {
